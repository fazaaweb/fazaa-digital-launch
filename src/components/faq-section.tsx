...
export function FaqSection() {
  return (
    <section className="relative z-10 py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-background via-card/20 to-background">
      <div className="mx-auto max-w-4xl">
        ...
        {/* Accordion FAQ */}
        <div className="mt-14 rounded-3xl border border-white/10 bg-card/60 p-6 sm:p-8 backdrop-blur-xl shadow-xl">
          <Accordion type="single" collapsible className="w-full space-y-4">
            {faqs.map((faq, idx) => (
              ...
              >
                <AccordionTrigger className="text-right text-sm sm:text-base font-bold text-foreground hover:no-underline hover:text-primary-highlight">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="text-xs sm:text-sm leading-relaxed text-secondary-foreground/90 pt-2 pb-4">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
